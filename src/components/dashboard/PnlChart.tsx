...
export function PnlChart({ data, currency }: { data: BalancePoint[]; currency: string }) {
  if (data.length <= 1) {
    return (
      <div className="flex h-56 items-center justify-center rounded-xl border border-border bg-charcoal-light">
        <p className="font-mono text-sm text-slate-500">Pas encore assez de données réglées pour un graphique.</p>
      </div>
    )
  }
  ...
  return (
    <div className="rounded-xl border border-border bg-charcoal-light p-4">
      <p className="mb-2 text-xs uppercase tracking-wide text-slate-400">Évolution de la bankroll</p>
      <ResponsiveContainer width="100%" height={220}>
        <LineChart data={data}>
          ...
          <Tooltip
            ...
            formatter={(value) => [formatCurrency(Number(value), currency), 'Solde']}
          />
